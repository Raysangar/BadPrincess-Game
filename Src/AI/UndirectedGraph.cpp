...
void CUndirectedGraph::setAdjacent(unsigned int orig, unsigned int dest) {
	std::list<unsigned int>* neighbours = _adjacency[orig];
	// Si es null es porque nuevo
	if (neighbours == NULL) {
		neighbours = new std::list<unsigned int>();
...
*/
const std::list<unsigned int> &CUndirectedGraph::getNeighbours(unsigned int idNode) 
{
	assert(idNode < _nodes.size());
	std::list<unsigned int>* neighbours = _adjacency[idNode];
	if (neighbours == NULL) {
...
void CUndirectedGraph::deleteAll()
{
...
void CUndirectedGraph::resetGraph()
{
	deleteAll();
	_edgeCount = 0;
	_cost = CDynTriMat<float>();
}
