...
namespace Application 
{
	...
	*/
	...
	public:
		...
		*/
		CGameState(CBaseApplication *app,std::string name) : CApplicationState(app),_name(name), 
				_scene(0), _time(0) {}
		...
		*/
		...
		unsigned int getGameTime(){return _time;};

	protected:

		bool pauseReleased(const CEGUI::EventArgs& e);
		...
		*/
		...
		/**
		Apellido del nivel que tenemos que cargar
		*/
		std::string _name;
	}; // CGameState
