...
namespace Graphics 
{
	...
	CServer::CServer() : _root(0), _renderWindow(0), _activeScene(0), _dummyScene(0),fogOfWar(0)
	{
	...
	Ray CServer::getCameraToViewportRay(float screenx, float screeny){
		 assert(_activeScene);
		//Se obtiene la cámara de la escena acual 
		Graphics::CCamera* camera =  _activeScene->getCamera();


		//Se obtiene la cámara de Ogre y se llama a la función getCameraToViewportRay para obtener el rayo que es                     necesario lanzar
