...
namespace Graphics 
{
	...
	*/
	...
	public:
		...
		*/
		virtual void animationFinished(const std::string &animation, const bool blending) {}

	}; // CAnimatedEntityListener

	/**
	Clase que representa una entidad gráfica con animaciones. Especializa la clase
	Graphics::CMeshEntity por lo que contiene una referencia a una entidad de 
	...
	@date Julio, 2010
	*/
	class CAnimatedEntity : public CMeshEntity
	{
	public:
		...
		*/
		CAnimatedEntity(const std::string &name, const std::string &mesh):
			CMeshEntity(name,mesh), _currentAnimation(nullptr), _targetAnimation(nullptr), animationPaused(false) {}
		...
		*/
		...
		/**
		Funcion que realiza el blend
		*/
		void blend(float secs);
		...
		*/
		...
		void restartAnimation() { animationPaused = false; }
	...
	protected:
		...
		*/
		...
		/**
		Animacion entrante para el blend
		*/
		Ogre::AnimationState *_targetAnimation;

		/**
		Variable que controla el tiempo que se lleva de blending
		*/
		Ogre::Real _timeAcum;
