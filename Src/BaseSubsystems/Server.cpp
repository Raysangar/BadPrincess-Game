...
namespace BaseSubsystems 
{
	...
	*/
	...
	bool CServer::initScriptManager() {
		...
		ScriptManager::CScriptManager::GetSingleton().executeScript("MOUSE_EXCLUSIVE = true")
#endif
		return true;
	}
		...
		if(force || !_root->restoreConfig()) {
			...
			for(; it != rs.end(); ++it)
			{
				if((*it)->getName() == "Direct3D9 Rendering Subsystem")
				{
					...
					(*it)->setConfigOption("Video Mode","1366 x 768 @ 32-bit colour");
				}
			}
		}
		return true;
	...
	bool CServer::initOgreResources()
	{
		...
		try
		{
			...
			_renderWindow = _root->initialise(true,"Bad Princess");
					...
					SetClassLong (hwnd, GCL_HICON, 
						   (LONG)LoadIcon (hInst, MAKEINTRESOURCE (101)));
			 #endif
		...
		Options::Resolution resolution = gameOptions->getAvailableResolutions()->at(
			std::atoi(gameOptions->getAttribute("actualResolutionId").c_str()));

		bool fullScreen = (std::atoi(gameOptions->getAttribute("fullScreen").c_str()) == 1);
		_renderWindow->setFullscreen(fullScreen,resolution.first,resolution.second);
	...
	void CServer::reloadOgreResource()
	{
		  Ogre::ResourceGroupManager::getSingletonPtr()->clearResourceGroup("General");
		  Ogre::ResourceGroupManager::getSingletonPtr()->initialiseResourceGroup("General");
	}

	void CServer::setFullScreen(bool active, Options::Resolution resolution)
	{
		_renderWindow->setFullscreen(active, resolution.first, resolution.second);
		const OIS::MouseState &mouseStateOIS = _mouse->getMouseState();
		mouseStateOIS.width  = _root->getRenderSystem()->_getViewport()->getActualWidth();
		mouseStateOIS.height = _root->getRenderSystem()->_getViewport()->getActualHeight();

		CEGUI::System::getSingletonPtr()->notifyDisplaySizeChanged(CEGUI::Sizef((float)_root->getRenderSystem()->_getViewport()->getActualWidth(), 
			(float)_root->getRenderSystem()->_getViewport()->getActualHeight()));
		_renderWindow->resize(resolution.first, resolution.second);
		setOgreConfigOption("Full Screen", active ? "Yes" : "No");
	}

	void CServer::changeAntialiasing(int antialiasingLevel)
	{
		setOgreConfigOption("FSAA", std::to_string(antialiasingLevel));
	}

	void CServer::setVSync(bool active)
	{
		setOgreConfigOption("VSync", active ? "Yes" : "No");
	}

	void CServer::changeResolution(Options::Resolution resolution, bool fullScreen)
	{

		if (fullScreen)
			_renderWindow->setFullscreen(true, resolution.first, resolution.second);
		else
		{
			_renderWindow->resize(resolution.first, resolution.second);
			centerWindow();
		...
		std::string resolutionString = std::to_string(resolution.first) + " x " + std::to_string(resolution.second) + " @ 32-bit colour";
		setOgreConfigOption("Video Mode", resolutionString);
	}

	void CServer::centerWindow()
	{
		Ogre::Rect maximunResolution;
		HMONITOR hMonitor = MonitorFromWindow(HWND_DESKTOP, MONITOR_DEFAULTTONEAREST);
		MONITORINFO lpmi;
		lpmi.cbSize = sizeof(lpmi);
		if(GetMonitorInfo(hMonitor, &lpmi))
		{
			maximunResolution.left = lpmi.rcMonitor.left;
			maximunResolution.top = lpmi.rcMonitor.top;
			maximunResolution.right = lpmi.rcMonitor.right;
			maximunResolution.bottom = lpmi.rcMonitor.bottom;
		}
		_renderWindow->reposition((maximunResolution.width() - _renderWindow->getWidth())/2, (maximunResolution.height() - _renderWindow->getHeight())/2);
		
	}

	void CServer::setOgreConfigOption(std::string attributeName, std::string attributeValue)
	{
		_root->getRenderSystem()->setConfigOption(attributeName, attributeValue);
		_root->saveConfig();
	}
