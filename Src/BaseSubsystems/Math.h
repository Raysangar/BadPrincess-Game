...
namespace Math
{
	...
	*/
	...
	/**
	Crea un vector unitario de dirección en el plano XZ que corresponde
	al vector pasado por parámetro y girado 90 grados en el eje de la Y.

	@param direction Vector director
	@return Vector normal al vector director.
	*/
	static Vector3 getPerpendicular (const Vector3& direction) 
	{		
		Ogre::Quaternion q;		
		q.FromAngleAxis(Ogre::Degree(90),Vector3::UNIT_Y);
		return q * direction.normalisedCopy();
	}
