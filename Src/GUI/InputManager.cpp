...
namespace GUI{
	...
	bool CInputManager::mouseMoved(const OIS::MouseEvent &e) 
	{
		if (!_mouseListeners.empty()) 
		{
			...
			{
				if ((*it)->mouseMoved(_mouseState,alreadyManaged))
					alreadyManaged= true;
			}
		}
	...
	bool CInputManager::mousePressed(const OIS::MouseEvent &e, OIS::MouseButtonID button) 
	{
		if (!_mouseListeners.empty()) 
		{
			...
			for (; it != _mouseListeners.end(); it++) 
			{
				if ((*it)->mousePressed(_mouseState,alreadyManaged))
					alreadyManaged= true;
			}
		}
	...
	bool CInputManager::mouseReleased(const OIS::MouseEvent &e, OIS::MouseButtonID button) 
	{
		if (!_mouseListeners.empty()) 
		{
			...
			for (; it != _mouseListeners.end(); it++) 
			{
				if ((*it)->mouseReleased(_mouseState,alreadyManaged))
					alreadyManaged= true;
				}
		}
	...
	CMouseState& CInputManager::getLogicMouseState()
	{
		return _mouseState;
	}
} // namespace GUI