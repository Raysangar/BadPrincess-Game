	...
	*/
	...
	public:
		...
		*/
		CEntity *createEntity(const Map::CEntity *entityInfo,
							  CMap *map, const std::string &newName = "", Ogre::Vector3 *newPosition = NULL);
		...
		*/
		...
		/**
		Método que instancia una copia de una entidad pasada por referencia
		en una posición determinada.

		@param entity Entidad del juego que se quiere instanciar
		@param position Posición donde crear la entidad
		*/
	...
	protected:
		...
		*/
		...
		/**
		Tipo set de CEntity donde guardaremos los pendientes de borrar.
		*/
		typedef std::set<Logic::CEntity*> TEntitySet;

		/**
		Set de objetos pendientes de borrar.
		*/
		...
		Set de objetos pendientes de borrar.
		*/
		TEntitySet _deletedEntities;


		std::string CEntityFactory::getNextNameAvailable(Logic::CMap *map,std::string entityType);

		CEntity* instantiateEntityFromPool(Logic::CMap *map,std::string entityType);
