...
namespace Logic {
	...
	CMap* CMap::createMapFromFile(const std::string &filename)
	{
		...
		for(; it != end; it++)
		{
			if ((*it)->getType() == "WayPoint") {
				...
				int num = (*it)->getIntAttribute("WayPointsNum");
				for(int i = 0; i < num; ++i)
				{
					AI::CServer::getSingletonPtr()->addWaypoint((*it)->getVector3Attribute("WayPoint" + std::to_string(i)));
				}
	...
	void CMap::sendMessageToAll(const std::shared_ptr<NMessage> &message)
	{
		TEntityMap::const_iterator it=_entityMap.begin();
		
		for(;it != _entityMap.end();++it)
		{
			it->second->emitMessageN(message);
		}
	...
	void CMap::awake()
	{
		for (TEntityMap::iterator it = _entityMap.begin(); it != _entityMap.end(); ++it)
			if(!(*it).second->isGenerator())
				it->second->awake();
	}
